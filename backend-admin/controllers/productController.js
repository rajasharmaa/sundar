...
const createProduct = async (req, res) => {
  try {
    ...
    // Validate required fields - only name, category, and sizeOptions are truly required
    if (!name || name.trim() === '') {
...
const updateProduct = async (req, res) => {
  try {
    ...
    console.log('📦 sizeOptions type:', typeof req.body.sizeOptions);
    ...
    const fields = ['name', 'category', 'description', 'discount', 'material', 
                   'pressureRating', 'temperatureRange', 'standards', 'application', 'featured'];
    ...
    if (updates.sizeOptions || updates.price_100_percent || updates.price_50_percent) {
      ...
      console.log(`🗑️  Redis cache invalidated for product ${req.params.id}`);
    }
...
const bulkPriceUpdate = async (req, res) => {
  try {
    console.log('🔵 [BULK PRICE UPDATE] Request received:', req.body);
    ...
    console.log(`📦 [BULK PRICE UPDATE] Processing ${updates.length} updates`);
    ...
    for (const update of updates) {
      ...
      console.log(`🔄 [BULK PRICE UPDATE] Updating product ${productId}, size ${sizeIndex}, type ${priceType}, price ${newPrice}`);
      ...
      console.log(`✅ [BULK PRICE UPDATE] Updated ${priceType}% price to ${newPrice}`);
      
      // 🔥 CRITICAL FIX: Invalidate Redis cache in damoder-backend
      await invalidateProductCache(productId);
      console.log(`🗑️  [BULK PRICE UPDATE] Redis cache invalidated for product ${productId}`);
      
      results.push({ productId, success: true, priceType, newPrice });
    }
    
    console.log(`🎉 [BULK PRICE UPDATE] Completed. Success: ${results.filter(r => r.success).length}, Failed: ${results.filter(r => !r.success).length}`);
