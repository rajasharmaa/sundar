        ...
        if (token) {
            try {
                ...
                if (decoded && decoded.sub) {
                    socket.user = decoded;
                    logger.debug(`Socket ${socket.id} authenticated with token for user ${decoded.email}`);
                } else {
                    logger.warn(`Socket ${socket.id} provided invalid token payload`);
                    if (config.NODE_ENV === 'production') {
                        logger.warn(`Disconnecting unauthorized socket ${socket.id}`);
