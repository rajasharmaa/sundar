...
const client = new MongoClient(config.MONGODB_URI, {
    ...
    appname: 'sundar-corporation-api'    // Application identification
});
...
const createConnectionPromise = () => {
    return Promise.race([
            ...
            try {
                ...
                if (connectionRetries > 0) {
                    const delay = Math.min(
                        RETRY_DELAY_BASE * Math.pow(2, connectionRetries - 1) +
                ...
                await Promise.race([
                    ...
                    new Promise((_, reject) =>
                        setTimeout(() => reject(new Error('Primary connection timeout')), CONNECTION_TIMEOUT)
                    )
                ]);

                // 🔧 DATABASE VALIDATION
                const uriParts = config.MONGODB_URI.split('/');
                const dbName = uriParts[uriParts.length - 1]?.split('?')[0] || 'sundarCorporation';
                ...
                if (mongoose.connection.readyState !== 1) {
                    await mongoose.connect(config.MONGODB_URI, {
                        dbName: dbName, // ✅ Ensure Mongoose uses the correct database
                        serverSelectionTimeoutMS: 10000,
                        socketTimeoutMS: 45000,
                        maxPoolSize: config.NODE_ENV === 'production' ? 10 : 5,
                        autoIndex: false
                    });
        ...
        new Promise((_, reject) =>
            setTimeout(() => reject(new Error('Connection establishment timeout')), CONNECTION_TIMEOUT + 5000)
        )
    ]);
};
        ...
        const setupCollectionIdx = async (collectionName, indexes) => {
            ...
            try {
                ...
                for (const idx of indexes) {
                    ...
                    const conflictIdx = existingIndexes.find(e =>
                        JSON.stringify(e.key) === JSON.stringify(key) && e.name !== name
                    );
...
const checkDBHealth = async () => {
    try {
        if (!dbConnection) return { status: 'disconnected', error: 'No active connection' };
