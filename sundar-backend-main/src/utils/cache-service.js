...
class CacheService {
  ...
  async setProduct(id, product, ttl = 600) { // 10 minutes for products
    const key = this.generateKey('product', id);
  ...
  async setSession(sessionId, sessionData, ttl = 3600) { // 1 hour for sessions
    const key = this.generateKey('session', sessionId);
  ...
  async setStats(stats, ttl = 120) { // 2 minutes for stats
    const key = 'stats:overview';
    ...
    try {
      ...
      for (const category of categories) {
        const categoryProducts = await productsCollection.find(
          { category: category.name, active: { $ne: false } }
        )
          .limit(50)
          .toArray();
