...
const createInquiry = async (req, res) => {
  try {
    const { 
      name, email, phone, companyName, businessName, location,
      products, // Array of { productId, productName, quantity, selectedSize, priceType, unitPrice }
      inquiryType, message, pageSource,
      city, state, country, ipAddress,
      deviceType, browser, userAgent,
      businessType, priceType: businessPriceType
    } = req.body;
    ...
    console.info('[INQUIRY] Received request:', { 
      hasUser: !!user, 
    ...
    console.info('[INQUIRY] Inserting inquiry:', { 
      ...inquiryData, 
      userId: userId || 'guest',
    ...
    if (isHighValue) {
      setImmediate(() => {
        try {
          ...
          const productSummary = formattedProducts.map(p => 
            `${p.productName} (${p.quantity}x ₹${p.unitPrice})`
          ).join(', ');
            ...
            emailQueue.add('high-value-inquiry', {
              ...
              html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
                  ...
                  <p style="color: #999; font-size: 12px;">© ${new Date().getFullYear()} Sundar Corporation</p>
                </div>
              `
            }, {
              attempts: 3,
