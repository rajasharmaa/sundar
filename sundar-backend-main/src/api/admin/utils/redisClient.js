...
const connectRedis = async () => {
  if (redisClient) {
    console.info('✅ Redis already connected');
    return redisClient;
  }
  ...
  try {
    ...
    console.info('✅ Redis ready for cache invalidation');
...
const invalidateProductCache = async (productId) => {
  ...
  try {
    ...
    try {
      ...
      console.info('🔍 Current cache keys:', allKeys.length);
      if (allKeys.length > 0) {
        console.info('📋 Sample keys:', allKeys.slice(0, 10));
      }
    } catch (listError) {
    ...
    for (const pattern of patterns) {
      ...
      if (keys.length > 0) {
        ...
        console.info(`🗑️  Deleted ${deleted} keys matching pattern: ${pattern}`);
      } else {
        console.info(`ℹ️  No keys found for pattern: ${pattern}`);
      }
    }

    console.info(`✅ Cache invalidated for product ${productId}. Total keys deleted: ${deletedCount}`);
    return true;
  } catch (error) {
