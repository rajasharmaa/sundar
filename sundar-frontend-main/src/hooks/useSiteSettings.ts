...
export const DEFAULT_SETTINGS: SiteSettings = {
  logo: '/logo.png?v=' + new Date().getTime(),
    ...
    queryFn: async () => {
      try {
        ...
        if (response && response.success && response.data) {
          // Merge with DEFAULT_SETTINGS to ensure no missing properties (like logo) cause issues
...
export const getManufacturingBg = (settings: any) => {
  if (!settings) return '/manufacturing.jpg';
  return settings.globalBg || 
         settings.manufacturingBg || 
         settings.globalBanner || 
         settings.manufacturingBanner || 
         settings.productsBanner || 
         settings.manufacturingImage || 
         '/manufacturing.jpg';
};
