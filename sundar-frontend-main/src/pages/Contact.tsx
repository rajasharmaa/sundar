...
export function Contact() {
  ...
  const currentOrigin = typeof window !== 'undefined' ? window.location.origin : 'https://sundarcorporation.com';
  ...
  const jsonLdSchemas = [
    ...
    },
    {
      ...
      "name": "Contact Sundar Corporation | Packaging Bags Manufacturer"
    }
  ];

  const handleWhatsAppRedirect = () => {
    const dynamicPhone = WHATSAPP_PHONE.replace(/[+\s]/g, '');
    const defaultMsg = encodeURIComponent("Hello Sundar Corporation, I would like to submit a product query list.");
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <SEO
        title="Contact Us | Packaging Bags Manufacturer"
        description="Connect with Sundar Corporation, the leading Manufacturer, Wholesaler and Trader of HDPE, PP, BOPP, Bulk, and Jute Bags."
        noIndex={false}
          ...
          <ScrollReveal>
            <div className="bg-white rounded-2xl p-6 md:p-8 border border-slate-100 shadow-xl mb-8">
              <div className="max-w-2xl mb-6">
                <span className="text-[9px] font-black text-green-600 uppercase tracking-widest block mb-0.5">Process Flow</span>
                <h2 className="text-xl sm:text-2xl font-black text-slate-900 uppercase tracking-tight">Our Manufacturing Process</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 relative">
                {[
                  { step: "01", title: "Submit Details", desc: "Share your requirements for sizes, quantities, and bag types." },
                  { step: "02", title: "Requirement Review", desc: "Our team reviews your specifications and customization needs." },
                  { step: "03", title: "Get Price Quote", desc: "We provide competitive bulk rates and finalize the order." },
                  { step: "04", title: "Secure Dispatch", desc: "Quality-checked packaging solutions shipped promptly." }
                ].map((proc, i) => (
            ...
            <div className="bg-slate-900 text-white rounded-2xl p-6 md:p-8 overflow-hidden relative shadow-2xl my-8 border border-slate-850">
              ...
              <div className="grid md:grid-cols-2 gap-6 items-center relative z-10">
                <div className="space-y-4">
                  <span className="text-[9px] font-black text-green-400 uppercase tracking-widest block">Flagship Showroom</span>
                  <h2 className="text-2xl font-black uppercase tracking-tight">Visit Our Showroom</h2>
                  ...
                  <div className="flex flex-col gap-2 font-bold text-xs text-slate-400">
                    ...
                    <div className="flex items-center gap-1.5">
                      <CheckCircle2 size={14} className="text-emerald-400" />
                      <span>Instant bulk quote estimation compiled on site</span>
                    </div>
                  </div>
                </div>
                {/* Visual Representation Card */}
                <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col gap-3">
                  <div className="h-36 rounded-xl bg-gradient-to-br from-green-600/30 to-cyan-500/20 flex flex-col items-center justify-center border border-white/5 relative overflow-hidden group">
                    <Building2 size={48} className="text-green-400 opacity-60 group-hover:scale-110 transition-transform duration-500" />
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-3 bg-white/5 rounded-xl border border-white/5 text-center">
                      <span className="text-sm font-black text-green-400">Ready Inventory</span>
                      <p className="text-[8px] text-slate-455 uppercase font-black tracking-widest mt-0.5">Warehouse Status</p>
                    </div>
                    <div className="p-3 bg-white/5 rounded-xl border border-white/5 text-center">
                      <span className="text-sm font-black text-green-400">HDPE / PP Bags</span>
                      <p className="text-[8px] text-slate-455 uppercase font-black tracking-widest mt-0.5">Packaging Solutions</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="fixed bottom-6 right-4 sm:right-6 z-[999] flex flex-col gap-2.5">
        ...
        <motion.a
          ...
          whileHover={{ scale: 1.08, y: -2 }}
          whileTap={{ scale: 0.95 }}
          className="w-11 h-11 sm:w-13 sm:h-13 bg-green-600 text-white rounded-full flex items-center justify-center shadow-2xl hover:bg-green-700 transition-colors border border-green-500/20 cursor-pointer flex-shrink-0"
