...
const FAQ = () => {
      ...
      ordering: [
         {
            ...
            a: 'MOQ varies by product. For standard items like bags and bags, we support both small and bulk orders. Please contact our sales team for specific item requirements.'
         },
         {
      ...
      technical: [
         {
            ...
            a: 'Yes, we provide custom manufacturing services for PP/HDPE woven bags, BOPP bags, and specialized packaging components based on your design specifications.'
         },
         {
            q: 'What pressure ratings do your sacks support?',
            a: 'Our sacks range from standard Class 150 up to high-pressure Class 2500, suitable for various industrial applications from HVAC to oil and gas.'
         }
      ]
   };
   ...
   return (
      <>
         <Helmet>
            <title>FAQs - Sundar Corporation | Help & Support Center</title>
            <meta name="description" content="Find answers to common questions about industrial bags, bags, ordering, and shipping at Sundar Corporation support center." />
         </Helmet>
         ...
         <main className="relative z-10 pt-24 pb-20">
            ...
            <section className="max-w-7xl mx-auto px-6 mb-20">
               <div className="relative p-12 md:p-20 bg-gray-900 rounded-[3rem] overflow-hidden">
                  <div className="absolute top-0 right-0 w-96 h-96 bg-green-600/20 rounded-full blur-[100px]" />
                  <div className="relative z-10 max-w-2xl">
                     <motion.div
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
                        className="flex items-center gap-2 text-green-400 font-black uppercase tracking-widest text-xs mb-6"
                     >
                        <HelpCircle size={14} />
                     ...
                     <div className="relative group max-w-md">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-green-500 transition-colors" />
                        <input
                           type="text"
                           placeholder="Search your question..."
                           className="w-full pl-12 pr-6 py-4 bg-white/5 border border-white/10 rounded-2xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-green-500 transition-all"
                        />
                     </div>
                  </div>
               </div>
                        ...
                        <div className="space-y-3">
                           {categories.map((cat) => (
                              <button
                                 ...
                                 className={`w-full flex items-center justify-between p-5 rounded-2xl font-black uppercase tracking-tighter text-sm transition-all ${activeTab === cat.id
                                       ? 'bg-green-600 text-white shadow-xl shadow-green-600/20 translate-x-2'
                     ...
                     <div className="space-y-4">
                        {faqs[activeTab as keyof typeof faqs].map((faq, i) => (
                           <ScrollReveal key={faq.q} delay={i * 0.1} distance={20}>
                              <div
                                 className={`bg-white rounded-[2rem] border transition-all duration-500 ${openIndex === i ? 'border-green-200 shadow-xl' : 'border-gray-100'
                                    }`}
                              >
                                 <button
                                 ...
                                 >
                                    <span className={`text-xl font-black uppercase tracking-tighter transition-colors ${openIndex === i ? 'text-green-600' : 'text-gray-900'
                                       }`}>
                                       {faq.q}
                                    </span>
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${openIndex === i ? 'bg-green-600 text-white rotate-180' : 'bg-gray-50 text-gray-400'
                                       }`}>
            ...
            <section className="max-w-7xl mx-auto px-6">
               <div className="grid md:grid-cols-3 gap-8">
                  {[
                     ...
                     { icon: Mail, title: 'Email Support', info: 'Response time: 24 hours', link: 'mailto:info@Sundar CorporationCorporation.com' }
                  ].map((item, i) => (
                     <ScrollReveal key={item.title} delay={i * 0.1} distance={30}>
                        <a
                           href={item.link}
                           className="group bg-white p-10 rounded-[3rem] border border-gray-100 hover:border-green-600 transition-all duration-500 hover:-translate-y-2 block h-full"
                        >
