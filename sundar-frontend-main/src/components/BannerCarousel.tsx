...
const getThemeColors = (color: string) => {
  switch(color) {
    ...
    case 'blue':
    default: return { bg: 'from-[#0f172a] via-[#1e3a8a] to-[#0f172a]', blob: 'bg-green-500/20', line: 'bg-green-400', grad1: ['#3b82f6', '#1d4ed8'], grad2: ['#60a5fa', '#2563eb'], text: 'text-green-300' };
  }
};
          ...
          <div className={`${bannerType === 'abstract_split' ? 'w-full md:w-1/2 px-6 sm:px-8 md:px-16 lg:px-20 pt-10 md:pt-0 flex-1 md:h-full' : 'w-full px-6 sm:px-8 md:px-16 lg:px-20 h-full'} flex flex-col justify-center relative pointer-events-none`}>
            <div className={`max-w-xl flex flex-col ${alignment} pointer-events-auto`}>
              ...
              {bannerType === 'full_image' && (
                <motion.div 
                  ...
                  className={`absolute ${currentBanner.textAlign === 'center' ? 'hidden' : currentBanner.textAlign === 'right' ? '-right-6 md:-right-10' : '-left-6 md:-left-10'} top-0 w-1 bg-gradient-to-b from-green-500 to-cyan-400 rounded-full`}
                />
              )}
                ...
                >
                  <Link 
                    ...
                    className={bannerType === 'abstract_split'
                      ...
                      : `group relative inline-flex items-center justify-center gap-3 px-8 py-4 bg-gradient-to-r from-green-600 -green- text-white font-black rounded-full overflow-hidden transition-all duration-300 hover:shadow-[0_0_40px_rgba(79,70,229,0.5)] hover:scale-105 active:scale-95 uppercase tracking-widest text-xs md:text-sm border border-white/10`
                    }
                  >
            ...
            {bannerType === 'abstract_split' && (
              <div className="absolute bottom-6 left-8 md:left-16 lg:left-20 text-xs text-white/50 font-light tracking-widest hidden md:block">
                www.Sundar CorporationCorporation.com
              </div>
            )}
          </div>
            ...
            {activeBanners.map((_, idx) => (
              <button
                ...
                className={`transition-all duration-300 rounded-full ${bannerType === 'abstract_split'
                  ...
                  : (idx === currentIndex ? 'bg-green-400 w-8 h-2 shadow-[0_0_10px_rgba(96,165,250,0.8)]' : 'bg-white/40 hover:bg-white/70 w-2 h-2')
                }`}
              />
            ))}
          </div>
