...
export const Breadcrumbs = ({ items, className = '' }: BreadcrumbsProps) => {
  ...
  const structuredData = {
    ...
    itemListElement: breadcrumbItems.map((item, index) => ({
      ...
      position: index + 1,
      name: item.label,
      item: item.href ? `https://Sundar CorporationCorporation.com${item.href}` : undefined
    }))
  };
            ...
            return (
              ...
              >
                ...
                {isLast ? (
                  ...
                  <Link
                    to={item.href!}
                    className="text-gray-600 hover:text-green-600 transition-colors px-2 py-1 rounded-md hover:bg-gray-50 max-w-[100px] xs:max-w-[120px] sm:max-w-[150px] truncate block"
                    title={item.label}
                  >
                    {item.href === '/' ? (
                      <span className="flex items-center gap-1">
