...
const ShopSlider = () => {
  ...
  const photos = settings?.shopPhotos?.length > 0 ? settings.shopPhotos : shopPhotos;
  const [index, setIndex] = useState(0);

  const next = useCallback(() => setIndex((prev) => (prev + 1) % photos.length), [photos.length]);
  const prev = useCallback(() => setIndex((prev) => (prev - 1 + photos.length) % photos.length), [photos.length]);
          ...
          <div className="lg:w-1/2">
            ...
            >
              ...
              <h2 className="text-4xl sm:text-5xl md:text-7xl font-black text-gray-900 leading-none tracking-tighter">
                OUR <span className="text-green-600">HUB</span> OF <br /> EXCELLENCE
              </h2>
              <p className="text-xl text-gray-600 font-medium max-w-lg">
                Step inside the engine room of Sundar Corporation. From our modern showroom to our advanced warehouse.
              </p>
              
              <div className="flex gap-4 pt-8">
                <button onClick={prev} className="w-14 h-14 rounded-full border-2 border-gray-200 flex items-center justify-center hover:bg-green-600 hover:border-green-600 hover:text-white transition-all duration-300">
                  <ChevronLeft size={24} />
                </button>
                <button onClick={next} className="w-14 h-14 rounded-full border-2 border-gray-200 flex items-center justify-center hover:bg-green-600 hover:border-green-600 hover:text-white transition-all duration-300">
                  ...
                  >
                    <div className="h-full w-full bg-white rounded-[3rem] p-4 shadow-2xl border border-white/50 relative overflow-hidden group">
                      ...
                      <div className="absolute inset-0 bg-white/10 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center">
                         <div className="bg-green-600 text-white px-6 py-2 rounded-full font-bold text-xs uppercase tracking-widest shadow-xl">
                            NEXT PHOTO
                         </div>
                      </div>
                    </div>
