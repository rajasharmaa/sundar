...
const Label = ({ children, required, className = "" }: { children: React.ReactNode; required?: boolean; className?: string }) => (
  <label className={`block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 ml-1 ${className}`}>
    {children}
    ...
    <div className="w-full">
      ...
      <div className="relative group">
        ...
        <input
          {...props}
          className={`
            w-full ${icon ? 'pl-11' : 'px-4'} py-3.5 bg-slate-50 border-2 rounded-xl outline-none transition-all duration-300
            ${error
              ? 'border-red-100 bg-red-50/30 focus:border-red-500 focus:bg-white'
              : 'border-slate-100 hover:border-slate-200 focus:border-blue-600 focus:bg-white focus:shadow-lg focus:shadow-blue-600/5'}
            placeholder:text-slate-300 text-slate-700 font-semibold text-sm
            ${className}
          `}
        />
      </div>
    ...
    <div className="w-full">
      ...
      <div className="relative group">
        ...
        <select
          {...props}
          className={`
            w-full ${icon ? 'pl-11' : 'px-4'} py-3.5 bg-slate-50 border-2 rounded-xl outline-none transition-all duration-300 appearance-none
            ${error
              ? 'border-red-100 bg-red-50/30 focus:border-red-500 focus:bg-white'
              : 'border-slate-100 hover:border-slate-200 focus:border-blue-600 focus:bg-white focus:shadow-lg focus:shadow-blue-600/5'}
  ...
  return (
    <div className="w-full">
      ...
      <textarea
        {...props}
        className={`
          w-full px-4 py-3.5 bg-slate-50 border-2 rounded-xl outline-none transition-all duration-300 min-h-[120px]
          ${error
            ? 'border-red-100 bg-red-50/30 focus:border-red-500 focus:bg-white'
            : 'border-slate-100 hover:border-slate-200 focus:border-blue-600 focus:bg-white focus:shadow-lg focus:shadow-blue-600/5'}
          placeholder:text-slate-300 text-slate-700 font-semibold text-sm resize-none
          ${className}
        `}
      />
      <ErrorMessage error={error} />
