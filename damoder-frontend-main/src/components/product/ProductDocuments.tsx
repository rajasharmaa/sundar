  ...
  const handleDownload = (docName: string, docLabel: string) => {
    try {
      const formattedSpecs = [
        ...
        product.pressureRating ? `Pressure Rating: ${product.pressureRating}` : '',
        product.temperatureRange ? `Temperature Range: ${product.temperatureRange}` : '',
      ...
      const content = `======================================================================
DAMODAR TRADERS - OFFICIAL PRODUCT SPECIFICATION LOG
======================================================================
...
Website:       https://damodartraders.com

Address:
1st floor, 37 Ellora plaza, 3, Maharani Rd, Indore, MP 452007
======================================================================
Thank you for partnering with Damodar Traders. For project-level custom sizing,
    ...
    <div id="product-documents" className="bg-white rounded-3xl border border-slate-200/60 shadow-xl shadow-slate-100/30 p-6 sm:p-8 md:p-10 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {documents.map((doc, idx) => (
          <div
            key={idx}
            className="flex items-center justify-between p-4 bg-slate-50 hover:bg-slate-100/80 border border-slate-200/60 hover:border-blue-300 rounded-2xl transition-all group cursor-pointer"
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-sm font-extrabold text-slate-800 block group-hover:text-blue-700 transition-colors leading-tight">
                  {doc.label}
                </span>
