...
const StickyMobileActionBar = ({
  ...
  return (
    ...
    >
      {/* Product Info Bar */}
      <div className="bg-gradient-to-r from-blue-50 to-cyan-50 px-4 py-2 border-b border-gray-100">
        <div className="flex items-center justify-between gap-2">
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-gray-700 truncate">{productName}</p>
            {productPrice && (
              <p className="text-sm font-bold text-blue-600">₹{productPrice.toLocaleString()}</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2 p-3 bg-white">
        ...
        <motion.button
          ...
          className="flex flex-col items-center justify-center py-2.5 px-2 bg-gradient-to-br from-blue-600 to-cyan-600 text-white rounded-xl shadow-md active:scale-[0.95] transition-all min-h-[56px]"
        ...
        >
          ...
          <span className="text-[10px] font-medium">{isInWishlist ? 'Saved' : 'Save'}</span>
        </motion.button>
      </div>
    </motion.div>
  );
