  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Link
          to="/products"
          className="group inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-700 hover:border-blue-600 hover:text-blue-600 transition-all duration-300 shadow-sm hover:shadow-md ml-10 sm:ml-0"
        >
      ...
      <div className="relative">
        ...
        <div className="sm:hidden flex justify-center gap-1.5 mt-2">
          {relatedProducts.slice(0, Math.min(relatedProducts.length, 5)).map((_, i) => (
            <div key={i} className={`h-1 rounded-full transition-all duration-300 ${i === 0 ? 'w-4 bg-blue-600' : 'w-1 bg-gray-300'}`} />
          ))}
        </div>
      </div>
    </div>
