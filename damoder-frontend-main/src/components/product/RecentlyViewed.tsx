...
export const RecentlyViewed = ({ limit = 5, className = '', showClearButton = true }: RecentlyViewedProps) => {
  ...
  const getBadgeStyles = (category: string) => {
    ...
    if (cat.includes('pipe')) return 'bg-blue-100 text-blue-700 border-blue-200';
    if (cat.includes('fitting')) return 'bg-emerald-100 text-emerald-700 border-emerald-200';
    if (cat.includes('valve')) return 'bg-red-100 text-red-700 border-red-200';
          ...
          {recentlyViewed.map((product, index) => {
            ...
            return (
              <motion.div
                ...
                className="group relative flex flex-col h-full bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:border-blue-100 transition-all duration-300 overflow-hidden"
              >
                ...
                >
                  ...
                  {/* Hover Overlay CTA */}
                  <div className="absolute inset-0 bg-blue-900/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                ...
                <div className="p-4 flex flex-col flex-1">
                  <Link to={`/products/${product.id || product._id}`} className="block">
                    <h4 className="font-bold text-gray-900 text-sm line-clamp-2 group-hover:text-blue-600 transition-colors leading-tight mb-2 min-h-[2.5rem]">
                      {product.name}
                    </h4>
                  </Link>
