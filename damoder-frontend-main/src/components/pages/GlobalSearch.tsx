        ...
        <div className="relative w-full max-w-[320px]" ref={searchRef}>
            <form onSubmit={handleSearch} className="group relative">
                <input
                    ...
                    className="w-full pl-11 pr-16 py-2.5 bg-gray-50/80 border border-gray-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all duration-300 text-sm placeholder:text-gray-400"
                    value={displayQuery}
                    onChange={(e) => {
            ...
            {showDropdown && (
                <div
                    id="search-results"
                    className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-2xl border border-blue-100 overflow-hidden z-[110] animate-in fade-in animate-out fade-out slide-in-from-top-2 duration-200 origin-top min-w-[300px]"
                >
                    <div className="max-h-[60vh] overflow-y-auto">
                        {isLoading ? (
                            <div className="flex items-center justify-center py-6 text-blue-500">
                            ...
                            <div className="p-1 space-y-0.5">
                                {results.map((item: any, index) => (
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-3 p-2 rounded-lg text-left transition-all group ${selectedIndex === index ? 'bg-blue-50' : 'hover:bg-gray-50'
                                            }`}
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="font-bold text-gray-900 text-sm truncate group-hover:text-blue-600 transition-colors">{item.name}</div>
                    ...
                    {results.length > 0 && (
                        <div className="p-2 bg-gray-50 border-t border-gray-100">
                            <button
                                ...
                                className="w-full py-2.5 bg-blue-600 text-white text-xs font-bold hover:bg-blue-700 transition-colors flex items-center justify-center gap-2 rounded-lg"
                            >
