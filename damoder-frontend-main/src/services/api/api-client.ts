...
export const api = {
  auth: {
    ...
    status: () => apiClient.get<ApiResponse<AuthResponse['data']>>('/auth/status').then(res => res.data),
  ...
  products: {
    getAll: () => apiClient.get<ApiResponse<Product[]>>('/products?unlimited=true').then(res => {
      // Handle both response formats: { data: [...] } and raw array [...]
      const result = res.data;
      return Array.isArray(result) ? result : (result?.data || []);
    }),
    getAllRaw: () => apiClient.get<ApiResponse<Product[]>>('/products'),
    getPopular: () => apiClient.get<ApiResponse<Product[]>>('/products/popular').then(res => {
      // Handle both response formats: { data: [...] } and raw array [...]
      const result = res.data;
      // Popular products endpoint returns { success: true, data: [...] }
    ...
    getById: (id: string, skipCache = false) => apiClient.get<ApiResponse<Product>>(`/products/${id}`, {
      ...
      logger.debug('Product API response:', {
        ...
        hasProduct: !!(res.data?.data || res.data),
        productId: (res.data?.data as any)?.id || (res.data?.data as any)?._id
      });
      return res.data;
    }).catch(error => {
    ...
    getByCategory: (category: string) => apiClient.get<ApiResponse<Product[]>>(`/products/category/${category}`).then(res => {
      // Handle both response formats: { data: [...] } and raw array [...]
      const result = res.data;
      return Array.isArray(result) ? result : (result?.data || []);
    }),
    search: (query: string) => apiClient.get<ApiResponse<Product[]>>(`/products/search?q=${encodeURIComponent(query)}`).then(res => {
      // Handle both response formats: { data: [...] } and raw array [...]
      const result = res.data;
      return Array.isArray(result) ? result : (result?.data || []);
    }),
    getSuggestions: (query: string) => apiClient.get<ApiResponse<Product[]>>(`/products/search/suggestions?query=${encodeURIComponent(query)}`).then(res => {
      // Handle both response formats: { data: [...] } and raw array [...]
      const result = res.data;
      return Array.isArray(result) ? result : (result?.data || []);
    }),
    getRelated: (id: string, limit?: number) => apiClient.get<ApiResponse<Product[]>>(`/products/${id}/related${limit ? `?limit=${limit}` : ''}`).then(res => {
      const result = res.data;
      return Array.isArray(result) ? result : (result?.data || []);
    }),
    getPopularInCategory: (category: string, limit?: number, exclude?: string) => apiClient.get<ApiResponse<Product[]>>(`/products/category/${category}/popular?limit=${limit || 4}${exclude ? `&exclude=${exclude}` : ''}`).then(res => {
      const result = res.data;
      return Array.isArray(result) ? result : (result?.data || []);
    }),
  ...
  categories: {
    getAll: () => apiClient.get<ApiResponse<any[]>>('/categories').then(res => {
      const result = res.data;
      return Array.isArray(result) ? result : (result?.data || []);
    })
  },
  user: {
    getProfile: () => apiClient.get<ApiResponse<UserResponse['data']>>('/users/profile').then(res => res.data),
    updateProfile: (data: Partial<UserResponse['data']>) => apiClient.put<ApiResponse<UserResponse['data']>>('/users/profile', data).then(res => res.data),
    // Recently viewed products
    getRecentlyViewed: (limit?: number) => apiClient.get<ApiResponse<Product[]>>(`/users/recently-viewed${limit ? `?limit=${limit}` : ''}`).then(res => {
      const result = res.data;
      return Array.isArray(result) ? result : (result?.data || []);
    }),
    trackRecentlyViewed: (productId: string) => apiClient.post<ApiResponse<{ success: boolean; message: string }>>('/users/recently-viewed', { productId }).then(res => res.data),
    clearRecentlyViewed: () => apiClient.delete<ApiResponse<{ success: boolean; message: string }>>('/users/recently-viewed').then(res => res.data)
  },
  wishlist: {
    get: () => apiClient.get<ApiResponse<WishlistResponse['data']>>('/wishlist').then(res => res.data),
    add: (productId: string) => apiClient.post<ApiResponse<WishlistResponse['data']>>('/wishlist', { productId }).then(res => res.data),
    remove: (productId: string) => apiClient.delete<ApiResponse<WishlistResponse['data']>>(`/wishlist/${productId}`).then(res => res.data)
  },
  rfq: {
  ...
  metrics: {
    getHealth: () => apiClient.get<ApiResponse<any>>('/metrics/health').then(res => res.data),
    getStats: () => apiClient.get<ApiResponse<any>>('/metrics/stats').then(res => res.data)
  },
  settings: {
    get: () => apiClient.get<{ success: boolean; data: SiteSettings }>('/settings').then(res => res.data),
