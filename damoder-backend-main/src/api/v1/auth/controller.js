        ...
        if (timeSinceRevoked > 10000) { // after 10-second grace window
          ...
          try {
            ...
            await db.collection('refresh_tokens').updateMany(
              { userId: refreshTokenRecord.userId, revoked: false },
              {
                $set: {
                  revoked: true,
                  revokedAt: new Date(),
...
const forgotPassword = async (req, res) => {
  try {
    ...
    if (!user) return res.json({ message: 'Instructions sent if account exists.', success: true });
...
const handleGoogleCallback = async (req, res) => {
  try {
    ...
    if (error) return res.redirect(`${frontendUrl}/login?error=oauth_error`);
    ...
    if (!tokenResponse.ok) return res.redirect(`${frontendUrl}/login?error=token_exchange_failed`);
