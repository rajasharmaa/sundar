...
app.use((req, res, next) => {
  ...
  const hasForbiddenHeaders = forbiddenHeadersFromFrontend.some(header =>
  ...
  if (hasForbiddenHeaders) {
    logger.warn('❌ Blocked request with forbidden security headers from frontend', {
      forbiddenHeaders: forbiddenHeadersFromFrontend.filter(h =>
        req.headers[h.toLowerCase()] || req.headers[h]
      ),
      ip: req.ip,
