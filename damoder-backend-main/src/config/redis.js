  ...
  socket: {
    ...
    reconnectStrategy: (retries) => {
      ...
      const baseDelay = Math.min(
        Math.pow(1.8, retries) * 1000 + Math.random() * 3000,
        120000 // Maximum 120s delay for cold start scenarios
      );
  ...
  try {
    ...
    await Promise.race([
      ...
      new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Redis connection timeout')), 25000)
      )
    ]);
