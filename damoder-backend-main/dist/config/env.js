...
const envSchema = zod.object({
    ...
    FRONTEND_URL: zod.string().url().refine(url => process.env.NODE_ENV !== 'production' || url.startsWith('https://'), 'FRONTEND_URL must use HTTPS in production'),
    BACKEND_URL: zod.string().url().optional().or(zod.string().startsWith('http://localhost')).optional(),
    // Database Connections
    MONGODB_URI: zod.string().refine(uri => uri.startsWith('mongodb://') || uri.startsWith('mongodb+srv://'), 'Must be valid MongoDB connection string').refine(uri => process.env.NODE_ENV !== 'production' ||
        (!uri.includes('localhost') && !uri.includes('127.0.0.1')), 'Production MongoDB URI cannot contain localhost addresses'),
    REDIS_URL: zod.string().url().optional().refine(url => !url || (process.env.NODE_ENV !== 'production' || url.startsWith('rediss://')), 'Production Redis must use rediss:// (TLS) protocol'),
    // Proxy Configuration
    ...
    // CORS Configuration
    ALLOWED_ORIGINS: zod.string().optional().transform(val => val ? val.split(',').map(origin => origin.trim()) : []),
