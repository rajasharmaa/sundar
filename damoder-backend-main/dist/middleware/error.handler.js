...
const errorHandler = (err, req, res, next) => {
    ...
    if (err.name === 'ValidationError') {
        ...
        return sendErrorResponse(res, createError.badRequest(err.message), requestId, errorId);
    }
    // Handle MongoDB duplicate key errors
    if (err.code === 11000) {
        const field = Object.keys(err.keyValue)[0];
        logger.warn('Duplicate key error:', { ...errorWithContext, field });
        return sendErrorResponse(res, createError.conflict(`${field} already exists`), requestId, errorId);
    }
    // Handle JWT errors
    if (err.name === 'JsonWebTokenError' || err.name === 'TokenExpiredError') {
        logger.warn('JWT error:', errorWithContext);
        return sendErrorResponse(res, createError.unauthorized('Invalid or expired token'), requestId, errorId);
    }
    // Handle mongoose validation errors
    if (err.name === 'ValidationError') {
        const messages = Object.values(err.errors).map(e => e.message);
        logger.warn('Mongoose validation error:', errorWithContext);
        return sendErrorResponse(res, createError.badRequest(messages.join(', ')), requestId, errorId);
    }
    ...
    if (process.env.NODE_ENV === 'production') {
        // Here you would integrate with services like Sentry, NewRelic, etc.
        // Example: Sentry.captureException(err, { contexts: { request: errorWithContext } });
        // For now, log to error file
        logger.errorToFile({
            errorId,
            message: err.message,
            stack: err.stack,
