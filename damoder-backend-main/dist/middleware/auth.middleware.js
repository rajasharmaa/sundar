...
const requireAuth = async (req, res, next) => {
    try {
        ...
        try {
            ...
            const user = await db.collection('users').findOne({ _id: decoded.sub }, { projection: { password: 0, __v: 0 } });
            if (!user) {
    ...
    try {
        ...
        if (token) {
            ...
            if (decoded) {
                try {
                    const db = await connectToDB();
                    const user = await db.collection('users').findOne({ _id: decoded.sub }, { projection: { password: 0 } });
                    if (user) {
                        req.user = {
                            id: user._id,
                            email: user.email,
