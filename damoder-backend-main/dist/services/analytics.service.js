...
const getDashboardAnalytics = async (options = {}) => {
    ...
    return await cacheService.getCached(cacheKey, async () => {
        ...
        const [productStats, categoryStats, inquiryStats, userStats, viewStats] = await Promise.all([
            getProductStats(db),
            getCategoryStats(db),
...
const getViewStats = async (db, options = {}) => {
    ...
    const productsCollection = db.collection('products');
    const mostViewed = await productsCollection.find({ active: { $ne: false } })
        .sort({ views: -1 })
        .limit(limit)
        .project({
        _id: 1,
...
const trackProductView = async (productId, userId = null) => {
    try {
        ...
        const result = await productsCollection.updateOne({ _id: new ObjectId(productId) }, { $inc: { views: 1 } });
