...
const COMMON_PASSWORDS_DEFAULT = [
    ...
    'damodar', 'trader', 'industrial', 'supplies',
...
function hasSequentialCharacters(password) {
    ...
    for (let i = 0; i <= lowerPassword.length - PASSWORD_REQUIREMENTS.MAX_SEQUENTIAL_CHARS; i++) {
        ...
        if (isSequentialString(substring))
            return true;
    }
    return false;
}
...
function generatePasswordFeedback(validations, password) {
    ...
    if (!validations.hasUppercase)
        missingClasses.push('uppercase letters');
    if (!validations.hasLowercase)
        missingClasses.push('lowercase letters');
    if (!validations.hasNumber)
        missingClasses.push('numbers');
    if (!validations.hasSpecial)
        missingClasses.push('special characters');
