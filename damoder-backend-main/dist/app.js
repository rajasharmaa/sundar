...
const initializeApp = async () => {
    try {
        logger.info('🚀 Initializing Damodar Traders API with enterprise-grade security...');
        ...
        try {
            ...
            const activeRes = await usersCollection.updateMany({ active: { $exists: true }, isActive: { $exists: false } }, [{ $set: { isActive: "$active" } }]);
            ...
            const failRes = await usersCollection.updateMany({ failedLoginAttempts: { $exists: true }, loginAttempts: { $exists: false } }, [{ $set: { loginAttempts: "$failedLoginAttempts" } }]);
            ...
            const loginRes = await usersCollection.updateMany({ lastLogin: { $exists: true }, lastLoginAt: { $exists: false } }, [{ $set: { lastLoginAt: "$lastLogin" } }]);
...
app.options('*', (req, res) => {
    ...
    res.header('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With, X-CSRF-Token, Accept, X-Request-ID, X-Server-Time, X-Client-Version, X-Cold-Start, X-Health-Check, X-Background-Check, X-Wishlist-Request, X-Refresh-Token, content-type, authorization, x-requested-with, x-csrf-token, accept, x-request-id, x-server-time, x-client-version, x-cold-start, x-health-check, x-background-check, x-wishlist-request, x-refresh-token');
...
// 🔥 APPLY HELMET WITH PRODUCTION SAFETY CHECKS
app.use((req, res, next) => {
    // 🔐 PREVENT HELMET HEADERS FROM FRONTEND REQUESTS
    const forbiddenHeadersFromFrontend = [
        'strict-transport-security',
    ...
    // Check if any forbidden headers are present in request
    const hasForbiddenHeaders = forbiddenHeadersFromFrontend.some(header => req.headers[header.toLowerCase()] || req.headers[header]);
    if (hasForbiddenHeaders) {
        logger.warn('❌ Blocked request with forbidden security headers from frontend', {
            forbiddenHeaders: forbiddenHeadersFromFrontend.filter(h => req.headers[h.toLowerCase()] || req.headers[h]),
            ip: req.ip,
            userAgent: req.headers['user-agent']?.substring(0, 100)
        });
        return res.status(400).json({
            error: 'Invalid request - security headers not allowed from frontend'
        });
    }
    next();
});
...
// 3. Cookie parser WITHOUT signature (for consistent unsigned cookie access)
...
const { checkBlacklist } = require('./middleware/blacklist.middleware');
...
app.get('/health', async (req, res) => {
    ...
    try {
        ...
        logger.warn('Health check error (returning 200 to prevent cascading failures):', err.message);
        // 🔥 NEVER RETURN 500 FOR HEALTH CHECKS - ALWAYS 200 OK
        res.status(200).json({
            status: 'ok',
            uptime: process.uptime(),
            timestamp: new Date().toISOString(),
...
// 🔧 EXPLICIT OPTIONS HANDLER FOR HEALTH ENDPOINT CORS
app.options('/health', (req, res) => {
    res.header('Access-Control-Allow-Origin', process.env.FRONTEND_URL || 'http://localhost:5173');
    res.header('Access-Control-Allow-Methods', 'GET, HEAD, OPTIONS');
    res.header('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With, X-CSRF-Token, Accept, Origin, X-Requested-With, X-Request-ID, X-Server-Time, X-Client-Version, X-Cold-Start, X-Health-Check, X-Background-Check, X-Wishlist-Request, X-Refresh-Token, content-type, authorization, x-requested-with, x-csrf-token, accept, origin, x-request-id, x-server-time, x-client-version, x-cold-start, x-health-check, x-background-check, x-wishlist-request, x-refresh-token');
    res.header('Access-Control-Allow-Credentials', 'true');
    res.status(200).end(); // Changed from 204 to 200 for consistency
});
...
app.post('/api/admin/products/bulk-upload/confirm', bulkUploadController.confirmBulkUpload);
app.get('/api/admin/products/bulk-upload/progress/:jobId', bulkUploadController.getUploadProgress);
app.get('/api/admin/products/bulk-upload/template', bulkUploadController.downloadTemplate);
...
// Root endpoint
app.get('/', (req, res) => {
    res.json({ message: 'Damodar Traders API v1', status: 'UP' });
});
app.use((req, res) => {
