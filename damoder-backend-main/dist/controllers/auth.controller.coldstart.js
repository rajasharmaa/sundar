...
const { generateAccessToken, generateRefreshToken, verifyRefreshToken, getTokenRemainingTime } = require('../utils/jwt.coldstart');
...
const refreshToken = async (req, res) => {
    try {
        ...
        if (user.locked) {
            ...
            await db.collection('refresh_tokens').updateMany({ userId: user._id, revoked: false }, {
                $set: {
                    revoked: true,
                    revokedAt: new Date(),
        ...
        try {
            // Revoke old token
            await db.collection('refresh_tokens').updateOne({ tokenHash }, {
                $set: {
                    revoked: true,
                    revokedAt: new Date(),
...
const getStatus = async (req, res) => {
    try {
        ...
        const user = await db.collection('users').findOne({ _id: new ObjectId(req.user.id) }, { projection: { password: 0, rememberToken: 0, resetToken: 0 } });
        if (!user) {
            return res.status(401).json({
                success: false,
                authenticated: false,
            ...
            try {
                ...
                if (remainingTime > 0) {
                    ...
                    if (redisClient && redisClient.isOpen) {
                        ...
                        await redisClient.set(`blacklist:token:${tokenHash}`, 'true', { EX: remainingTime });
                ...
                logger.warn('Failed to blacklist token during logout', { error: err.message });
            }
        }
        // Connect to database
        let db;
