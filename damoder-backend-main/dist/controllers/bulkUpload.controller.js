...
const bulkUpload = async (req, res) => {
    try {
        ...
        if (!req.file) {
            return sendErrorResponse(res, createError.badRequest('CSV file is required'), req.requestId);
        }
        ...
        if (products.length === 0) {
            return sendErrorResponse(res, createError.badRequest('No valid products found in CSV'), req.requestId);
        }
        ...
        return sendErrorResponse(res, createError.internalServerError('Failed to process CSV file'), req.requestId);
    }
};
/**
...
const confirmBulkUpload = async (req, res) => {
    try {
        ...
        if (!jobId) {
            return sendErrorResponse(res, createError.badRequest('Job ID is required'), req.requestId);
        }
        ...
        return sendErrorResponse(res, createError.internalServerError('Failed to confirm bulk upload'), req.requestId);
    }
};
/**
 * Get upload progress
...
const getUploadProgress = async (req, res) => {
    try {
        ...
        return sendErrorResponse(res, createError.internalServerError('Failed to get upload progress'), req.requestId);
    }
};
/**
...
const downloadTemplate = async (req, res) => {
    try {
        ...
        return sendErrorResponse(res, createError.internalServerError('Failed to generate template'), req.requestId);
    }
};
module.exports = {
    bulkUpload,
