...
class TokenRepository {
    ...
    async revokeToken(refreshToken) {
        try {
            const db = await connectToDB();
            const tokensCollection = db.collection('refresh_tokens');
            await tokensCollection.updateOne({ refreshToken }, {
                $set: {
                    isActive: false,
                    revokedAt: new Date()
                }
    ...
    async revokeUserTokens(userId) {
        try {
            const db = await connectToDB();
            const tokensCollection = db.collection('refresh_tokens');
            const result = await tokensCollection.updateMany({ userId, isActive: true }, {
                $set: {
                    isActive: false,
                    revokedAt: new Date()
                }
    ...
    async rotateToken(userId, oldToken, newToken, metadata = {}, rotationChainId = null) {
        ...
        try {
            ...
            await session.withTransaction(async () => {
                ...
                await tokensCollection.updateOne({ refreshToken: oldToken, userId }, {
                    $set: {
                        isActive: false,
                        rotatedAt: new Date(),
