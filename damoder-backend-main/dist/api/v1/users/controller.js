...
const updateProfile = async (req, res) => {
    try {
        ...
        if (req.body.avatarColor)
            updates.avatarColor = req.body.avatarColor.trim();
        if (req.body.avatarIcon)
            updates.avatarIcon = req.body.avatarIcon.trim();
        if (req.body.businessName)
            updates.businessName = req.body.businessName.trim();
        if (req.body.businessType)
            updates.businessType = req.body.businessType.trim();
        if (req.body.themeColor)
            updates.themeColor = req.body.themeColor.trim();
...
const trackRecentlyViewed = async (req, res) => {
    try {
        ...
        await db.collection('users').updateOne({ _id: new ObjectId(userId) }, { $set: { recentlyViewed, updatedAt: new Date() } });
...
const getRecentlyViewed = async (req, res) => {
    try {
        ...
        const orderedProducts = validIds.map(id => products.find(p => p._id.toString() === id)).filter(p => p);
